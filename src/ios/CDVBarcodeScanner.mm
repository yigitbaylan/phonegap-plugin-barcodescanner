    ...
    // GLOBALS
    NSString * borderColor = @"#ffffff";
    int borderThickness = 15;
    NSString * fontColor = @"#ffffff";
    int fontSize = 24;
    NSString * laserColor = @"#ff0000";
    NSString * prompt = @"Scan";
    ...
    - (UILabel*)placeText:(NSString *)theText:(int)theX:(int)theY;
    ...
    - (void)scan:(CDVInvokedUrlCommand*)command {
        ...
        if(options[@"borderColor"]){
            borderColor = [options valueForKey:@"borderColor"];
        }
        if(options[@"borderThickness"]){
            NSNumber * thinkness = [options valueForKey:@"borderThickness"];
            borderThickness = [thinkness intValue];
        }
        if(options[@"fontColor"]){
            fontColor = [options valueForKey:@"fontColor"];
        }
        if(options[@"fontSize"]){
            NSNumber * fontSizeDict = [options valueForKey:@"fontSize"];
            fontSize = [fontSizeDict intValue];
        }
        if(options[@"laserColor"]){
            laserColor = [options valueForKey:@"laserColor"];
        }
        if(options[@"prompt"]){
            prompt = [options valueForKey:@"prompt"];
        }
        if(options[@"showLaser"]){
            showLaser = [[options valueForKey:@"showLaser"] boolValue];
        }
    ...
    - (UIView*)buildOverlayView {
        ...
        [self createOverlay: overlayView];
        ...
        ;
        UILabel* label = [self placeText:prompt :50 :40];
    ...
    #define RETICLE_WIDTH    15.0f
    #define RETICLE_OFFSET   60.0f
    #define RETICLE_ALPHA     1.0f
    #define SCANNER_HEIGHT   250.0f
    #define SCANNER_WIDTH    100.0f
    ...
    - (UIImage*)buildReticleImage {
        ...
        NSString * scan = @"scan";
        if (self.processor.is1D && showLaser) {
            UIColor* color = [self colorFromHexString:laserColor];;
            CGContextSetStrokeColorWithColor(context, color.CGColor);
            CGContextSetLineWidth(context, .5f);
            CGContextBeginPath(context);
            CGFloat lineOffset = (CGFloat) (RETICLE_OFFSET+(0.25*(SCANNER_WIDTH)) - borderThickness);
        ...
        if (self.processor.is2D) {
            ...
            UIColor* color = [self colorFromHexString:borderColor];
            CGContextSetStrokeColorWithColor(context, color.CGColor);
            CGContextSetFillColorWithColor(context, color.CGColor);
            CGContextSetLineWidth(context, RETICLE_WIDTH);
            int width = CGBitmapContextGetWidth(context);
            int correctionMargin = borderThickness;
            
            CGRect rectTL = CGRectMake(
            RETICLE_OFFSET,
            RETICLE_OFFSET * 2,
            SCANNER_WIDTH / 2,
            correctionMargin);
            
            CGRect rectLT = CGRectMake(
            RETICLE_OFFSET,
            RETICLE_OFFSET * 2,
            correctionMargin,
            SCANNER_WIDTH / 2);
            ...
            CGRect rectTR = CGRectMake(
            width - RETICLE_OFFSET - (SCANNER_WIDTH / 2),
            RETICLE_OFFSET * 2,
            SCANNER_WIDTH / 2 ,
           correctionMargin);
            
            CGRect rectRT = CGRectMake(
            width - RETICLE_OFFSET - correctionMargin,
            RETICLE_OFFSET * 2,
            correctionMargin,
            SCANNER_WIDTH / 2);
            
            CGRect rectBL = CGRectMake(RETICLE_OFFSET,(RETICLE_OFFSET * 2) + SCANNER_HEIGHT - correctionMargin, SCANNER_WIDTH / 2 ,correctionMargin);
            
            CGRect rectLB = CGRectMake(
            RETICLE_OFFSET,
            (RETICLE_OFFSET * 2) + SCANNER_HEIGHT - SCANNER_WIDTH / 2,
            correctionMargin,
            SCANNER_WIDTH / 2);
            
            CGRect rectBR = CGRectMake(width - RETICLE_OFFSET - SCANNER_WIDTH / 2, (RETICLE_OFFSET * 2) + SCANNER_HEIGHT - correctionMargin, SCANNER_WIDTH / 2, correctionMargin);
            CGRect rectRB = CGRectMake(width - RETICLE_OFFSET - correctionMargin, (RETICLE_OFFSET * 2) + SCANNER_HEIGHT - SCANNER_WIDTH / 2, correctionMargin, SCANNER_WIDTH / 2);
            
    //        CGContextAddPath(context, bezierPath);
            CGContextFillRect(context, rectTL);
            CGContextFillRect(context, rectLT);
            CGContextFillRect(context, rectTR);
            CGContextFillRect(context, rectRT);
            
            
            CGContextFillRect(context, rectBL);
            CGContextFillRect(context, rectLB);
            CGContextFillRect(context, rectBR);
            CGContextFillRect(context, rectRB);
    ...
    -(UILabel *)placeText:(NSString *)theText:(int)theX:(int)theY {
        ...
        UIFont *textFont = [UIFont fontWithName:@"Helvetica" size:fontSize];
        CGFloat selfHeight=self.view.bounds.size.height;
        CGFloat selfWidth=self.view.bounds.size.width;
        // Position of the text
        textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, selfWidth, 200.0f)];

        // Set text attributes
        textLabel.textColor = [self colorFromHexString:fontColor];
    //    textLabel.backgroundColor = [UIColor orangeCol or];
    ...
    - (void) createOverlay:(UIView *) overlayView {
        //        overlay content
        int topX = 0;
        int topY = 0;
        int screenWidth = self.view.bounds.size.width;
        int screenHeight = self.view.bounds.size.height;
        int correctionMargin = 14.0f;
        float remainingHeight = screenHeight - (RETICLE_SIZE - SCANNER_HEIGHT);
        float correctHeight = remainingHeight / 2 + correctionMargin / 2;
        float bazzelWidth = RETICLE_OFFSET - correctionMargin;
        float bazzelHeight = SCANNER_HEIGHT - RETICLE_OFFSET - correctionMargin / 2;
        UIColor* overlayColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.4];
        
        
        CGRect top = CGRectMake(topX, topY, screenWidth, correctHeight);
        CGRect left = CGRectMake(topX, correctHeight, bazzelWidth, bazzelHeight);
        CGRect right = CGRectMake(screenWidth - bazzelWidth, correctHeight, bazzelWidth, bazzelHeight);
        CGRect bottom = CGRectMake(topX, correctHeight + bazzelHeight, screenWidth, correctHeight);
        
        UIView *vTop = [[UIView alloc] initWithFrame:top];
        UIView *vLeft = [[UIView alloc] initWithFrame:left];
        UIView *vRight = [[UIView alloc] initWithFrame:right];
        UIView *vBottom = [[UIView alloc] initWithFrame:bottom];
        
        vTop.backgroundColor = overlayColor;
        vLeft.backgroundColor = overlayColor;
        vRight.backgroundColor = overlayColor;
        vBottom.backgroundColor = overlayColor;
        
        [overlayView addSubview:vTop];
        [overlayView addSubview:vLeft];
        [overlayView addSubview:vRight];
        [overlayView addSubview:vBottom];
    }
